...
2. **P1 – performance table. CLOSED 2026-09-25** (`engine_designer/plans/2026-09-25_performance_reanchor.md`, report `validation_engines/reports/2026-09-25_performance_reanchor_before_after.txt`): bipropellant performance now runs on the equilibrium tables (c*, Isp and pe/pc at eps 2-250) with a cited η_c* 0.975 and a per-pair reverse-solved `ETA_CF`. RS-25 427 → 447 s (real 455), RD-180 319 → 326 s (real 338); Merlin-1D and Rutherford stay ~4.5 % low (one ETA_CF per pair; a kinetic/boundary-layer split would need the literature in `claude_lit/OPEN_QUESTIONS.md`). Original text: the LOX/LH2 γ/M columns in `combustion._TABLES` are effective performance numbers, not physical ones. The equilibrium tables also carry c* and Isp at ε 10/40/100, so a re-anchored performance path is possible. That changes every Isp spot check and `DEFAULT_ETA_CSTAR`, so it needs its own plan. Symptoms today: RS-25 vacuum Isp is 6% low (427 vs 455); RD-180 is 6% low (318 vs 338).
