...
**2026-09-23 cooling audit + refactor - read `engine_designer/COOLING_AUDIT.md` before
touching any cooling number.** What changed structurally:
- `physics/cooling.py`, `design.py` and `validate.py` are now PACKAGES (`cooling/`,
  `design/`, `validate/`). Every old name is re-exported, so `from engine_designer.physics
  import cooling; cooling.X` / `from ..physics.design import EngineDesign` still work, and
  `python3 -m engine_designer.physics.{cooling,validate}` still run.
- `design/`: `engine.py` = the `EngineDesign` dataclass + an orchestrator `_compute_pass`
  that runs 17 stage functions in `*_stage.py`. Values shared between stages live on a
  per-pass `PassState` `s` (`s.x`); stage-local values stay plain locals.
  `cooling_stage.thermal` runs the ONE unified per-station thermal solve
  (`cooling/thermal_solve.py`) on the real contour BEFORE the pump chain. Pumps, expander,
  margins, fatigue, structure and GUI keys all read that solve. Never add a second,
  independent wall-temperature or flux computation.
- Combustion-gas heat-transfer properties (frozen cp/mu/Pr, Tc) and coolant properties
  come from BAKED tables (`physics/property_data/*.json`, reader `physics/thermo_tables.py`),
  generated by the KEPT script `tools/property_tables/generate_property_tables.py`
  (Cantera + CoolProp; runs locally in a venv or on Colab; self-checks against Sutton
  Table 5-5). Regenerate the tables; never hand-edit them. Since the 2026-09-25 P1
  re-anchor the PERFORMANCE path reads them too (`combustion.performance_state` /
  `cf_vac_ideal` / `exit_pressure_ratio`: shifting-equilibrium c*, Isp and pe/pc at eps
  2-250, plus a frozen-expansion Isp column kept for a future kinetic-loss model);
  `DEFAULT_ETA_CSTAR` 0.975 (cited) x the ideal c*, per-pair `ETA_CF` (reverse-solved on
...
- `engine_designer/validation_engines/`: real-engine corpus (`build_corpus.py`, cited
  ...
  cited data and closes physics self-consistency probes. After a deliberate physics
  change: save a `--report --diff` into `validation_engines/reports/`, then `--snapshot`.
...
4. **This sandbox has no `$DISPLAY`.** `gui/app.py` and its Tkinter code can only be
   ...
   "turbo" colormap + selectable `FlowColorScale` (`scale_for_network`: "coolant" fit = the
   regen-jacket range, the GUI default / "streams" fit / "absolute"; `render_layers.
   build_batches(color_scale=)` recolors from per-vertex `scalar` at batch time, so a scale
   switch is a re-batch, not a rebuild), stream tubes, ring loops, `stream_inside_tube` = a drawn tube's own grid
